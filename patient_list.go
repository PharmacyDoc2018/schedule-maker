...
func initPatientLists(c *config) error {
	...
	if err != nil { //-- if no save data -> check for excel files
		...
		if err != nil {
			...
			for i, item := range excelPatientLists.Slices {
				isMissingList, foundPosition := func(c *config, item PatientList) (bool, int) { //-- only adds patient list from excel files if patient list for that day doesn't already exist
				...
				if isMissingList {
					...
					newPtList := excelPatientLists.Slices[i]
